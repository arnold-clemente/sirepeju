...
class DashboardController extends Controller
{
    ...
    public function gobernacion(Request $request)
    {
        ...
        $otorgaciones = DB::table('otorgacion_gobernacions')
        ->selectRaw('
            count(id) as cantidad,
            MONTH(fecha_resolucion) as mes
        ')
        ...
        $consulta_year = DB::table('otorgacion_gobernacions')
    ...
    public function prueba()
    {
        ...
        $gobernaciones = DB::table('otorgacion_gobernacions')
            ->join('institucions', 'otorgacion_gobernacions.institucion_id', '=', 'institucions.id')
            ->selectRaw('
                count(otorgacion_gobernacions.institucion_id) as cantidad,
                institucions.nombre as institucion,
                institucions.departamento as departamento,
                otorgacion_gobernacions.tipo as tipo
            ')
            ->whereIn('otorgacion_gobernacions.estado', [1])
            ->groupBy('institucions.nombre', 'institucions.departamento', 'otorgacion_gobernacions.tipo')
            ->get();
