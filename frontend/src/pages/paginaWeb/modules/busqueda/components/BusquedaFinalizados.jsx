...
import { getFinalizados } from 'api/pagina/tramitesApi';

import Loader from 'components/Loader'
import LoaderBuscando from 'components/LoaderBuscando';
import CardBuscado from './CardBuscado';
...
const BusquedaFinalizados = () => {
    ...
    const { search, entidad, codigo, sigla, resolucion } = parameters;

    const { isLoading, data: consulta, isError, error } = useQuery({
        queryKey: ['busqueda_finalizados'],
    ...
    const filterAvanzado = (e) => {
        ...
        const filtered = consulta.filter(registro => {
            if (
                ...
                registro.resolucion.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "").includes(resolucion.toLowerCase())
            ) {
                return registro;
            }
        });
    ...
    return (
        <>
            <ModalBuscar registro={mostrar} modal={modal} closeModal={closeModal} />
                                ...
                                <div className="col-md-6">
                                    <div className="form-floating">
                                        <input type="search" className="form-control" id="codigo" placeholder="Codigo de Tramite"
                                            name='resolucion' value={resolucion} onChange={handleInputChange} minLength='3' maxLength='100' />
