...
import { getFundadoresAdecuacion, createFundadorAde, updateFundadorAde, destroyFundadorAde } from 'api/modificacionAdecuacionApi';
import { updateAdecuacionModificacion } from 'api/modificacionAdecuacionApi';

const ModalUpdateAdecuacion = ({ registro, handleInputChange, modal, open, close }) => {
  const queryClient = useQueryClient();

  const [vista, setVista] = useState('adecuacion');
  ...
  const [fundador, setFundador] = useState({ id: 0, nombre_completo: '', ci: '', adecuacion_id: 0 });
  const [modalFundador, openFundador, closeFundador] = useModal();

  const { personalidad_juridica, adecuacion_id, domicilio_legal, estatuto_organico, reglamento_interno } = registro;
  ...
  useEffect(() => {
    if (adecuacion_id != 0) {
      setCargando(true);
      getFundadores.mutate(adecuacion_id);
    }
    setErrorval({});
  }, [adecuacion_id])
  ...
  const getFundadores = useMutation({
    mutationFn: getFundadoresAdecuacion,
    onSuccess: (response) => {
      open();
  ...
  const handleAdd = (e) => {
    ...
    setButton('add');
    setErrorfun({});
    const auxiliar = { id: 0, nombre_completo: '', ci: '', adecuacion_id: registro.adecuacion_id }
  ...
  const handleEdit = (e, fundadorEdit) => {
    ...
    if (fundadorEdit.ci === null) {
      auxiliar = { id: fundadorEdit.id, nombre_completo: fundadorEdit.nombre_completo, ci: '', adecuacion_id: fundadorEdit.adecuacion_id }
      setFundador(auxiliar);
    } else {
      auxiliar = { id: fundadorEdit.id, nombre_completo: fundadorEdit.nombre_completo, ci: fundadorEdit.ci, adecuacion_id: fundadorEdit.adecuacion_id }
      setFundador(auxiliar);
    }
    openFundador();
    setButton('edit');
  ...
  const handleSaved = (e) => {
    ...
    formData.append('adecuacion_id', adecuacion_id)
    Swal.fire({
      title: "Estas seguro?",
      text: "No podrás revertir esto...!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        setLoading(true);
        close();
        updatedAdecuacion.mutate(formData)
      }
    });
  }

  const updatedAdecuacion = useMutation({
    mutationFn: updateAdecuacionModificacion,
    onSuccess: (response) => {
      console.log(response)
      if (response.status === true) {
        queryClient.invalidateQueries('modificaciones_adecuacion')
      ...
      <>
        ...
        <ModalDiv isOpen={modal} closeModal={close} title={'Modificar'}>
          <div className='container-fluid mb-2'>
            <div className="row">
              <button onClick={() => setVista('adecuacion')} className='col-md-6 button_opcion_1'>Adecuacion</button>
...
export default ModalUpdateAdecuacion
